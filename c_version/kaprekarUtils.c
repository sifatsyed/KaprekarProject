...
int isKaprekar(int n) {
  ...
  if(n < 1) {
    return 0; //false is not a return type, so 0 should be used
  }

  int i;
  long square = n * (long) n;
  int numDigits = (int) log10(n) + 1;
  long modulus = 1; //any number multiplied by zero gives a zero, so the initial number should be 1
  long first, second;

  //for each possible "split" of the square...
  for(i=-3; i<=numDigits; i++) {  //numberOfDigits is not declared si instead numDigits should be used, also, the initial value needs to be changed for the program to pass all the test cases 
